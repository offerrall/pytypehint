...
def test_tuple_default_on_function_list_param_rejected():
    ...
    with pytest.raises(TypeError, match="tags: default: expected list, got tuple"):
        signature_of(f)
...
def test_recursive_tuple_default_rejected():
    with pytest.raises(TypeError, match="children: default: expected list, got tuple"):
        struct_of(Tree)
...
def test_deep_inner_tuple_default_rejected_with_index_path():
    ...
    with pytest.raises(TypeError, match=r"m: default: \[1\]: expected list, got tuple"):
        signature_of(f)
...
def test_float_field_with_int_default_still_fails():
    ...
    with pytest.raises(TypeError, match="x: default: expected float, got int"):
        struct_of(C)
...
def test_dict_default_on_dataclass_field_still_fails():
    ...
    with pytest.raises(TypeError, match="v: default: expected Inner, got dict"):
        struct_of(C)
